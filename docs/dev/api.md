...
The FastAPI application is defined in `beenative/www.py` and includes:

- **Automatic API documentation** at `/docs` (Swagger UI) and `/redoc` (ReDoc)
- **Static file serving** from `beenative/static/` via the `/static/` endpoint
...
Create a new route in `beenative/www.py`:
...
Static files are served from `beenative/static/`:
...
uvicorn beenative.www:app --reload --host 0.0.0.0 --port 8000
...
uvicorn beenative.www:app --host 0.0.0.0 --port 8000 --workers 4

# Or using gunicorn with uvicorn workers
gunicorn beenative.www:app -w 4 -k uvicorn.workers.UvicornWorker
```
