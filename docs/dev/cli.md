...
The CLI application is defined in `beenative/cli.py` and automatically configured as an entry point in `pyproject.toml`:

```toml
[project.scripts]
beenative = "beenative.cli:app"
```

After installation, the CLI is available as the `beenative` command.
...
```bash
beenative my-command --option value argument
```
...
Add a basic command to `beenative/cli.py`:
...
```bash
beenative process input.txt --output output.txt --verbose
beenative process input.txt -o output.txt -v
```
...
```bash
beenative export --count 50 --format yaml --output data.yaml
```
...
```python
from beenative.cli import syncify
import httpx
...
```python
from beenative.services.db import get_engine
...
```bash
beenative db migrate
beenative db seed
beenative user create john john@example.com
beenative user list
```
...
```python
from beenative.commands import database, users
...
from beenative.cli import app, syncify
import asyncio
...
def test_version_command_runs():
    ...
    assert "beenative" in result.stdout.lower()
