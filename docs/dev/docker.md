...
ENV APP_MODULE=beenative.www:app
...
```yaml
volumes:
  - "./beenative:/app/beenative"  # Source code
...
docker build -f dockerfile.www -t beenative-www:latest .

# Tag for registry
docker tag beenative-www:latest ghcr.io/your-org/beenative-www:latest

# Push to registry
docker push ghcr.io/your-org/beenative-www:latest
```
...
if [ ! -z "$CREATE_TEST_DATA" ]; then
  ...
  python -m beenative.cli test-data
fi
...
services:
  www:
    image: ghcr.io/your-org/beenative-www:latest
    restart: always
...
# Run a command
docker-compose exec www python -m beenative.cli version

# Check database connection
docker-compose exec www python -c "from beenative.services.db import engine; print(engine)"
```
...
3. Images are pushed to `ghcr.io/EXAMPLE/beenative`

### Pull Images from Registry

```bash
# Pull latest image
docker pull ghcr.io/EXAMPLE/beenative-www:latest

# Pull specific version
docker pull ghcr.io/EXAMPLE/beenative-www:v1.2.3

# Use in docker-compose
services:
  www:
    image: ghcr.io/EXAMPLE/beenative-www:latest
```
