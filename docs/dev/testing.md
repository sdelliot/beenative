...
pytest --cov=./beenative --cov-report=term-missing tests

# Generate HTML coverage report
pytest --cov=./beenative --cov-report=html tests
# Open htmlcov/index.html in your browser

# Show coverage for specific file
pytest --cov=./beenative/services/cache.py tests/services/test_cache.py
```
...
def test_version_command():
    ...
    assert "beenative" in result.stdout
```
...
```python
@pytest.mark.asyncio
@patch('beenative.services.cache.get_cached')
...
def test_settings_load():
    ...
    assert settings.project_name == "beenative"
...
pytest --cov=./beenative --cov-fail-under=80 tests
```
